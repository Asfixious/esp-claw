...
//! each self-contained generator. Today there is one — [`agent_manifests`], which
//! turns `resources/agents/<kind>/` into typed `AgentManifest` statics in
//! `$OUT_DIR/manifests.rs` (`include!`-d by `agent::manifest`). To add another
//! generator, give it its own module exposing a `generate(...)` entry and call it
//! here; keep this function free of generation logic.
//!
