...
| `BlockKind` | The canonical context kinds (e.g. `Soul`, `AssistantIdentity`, `UserProfile`, `GlobalMemory`, `AgentInstruction`, `ToolPolicy`, `ModePolicy`, `ActiveMode`, `ToolReminder`, `AgentMemory`, `SkillList`, `ReasoningEffort`, `RecentContext`, `OutputContract`) plus `Custom { band, scope, order, label }` for caller-defined blocks. |
