...
5. **Init capabilities**
    ...
    - **claw_im_gateway / cap_im_platform**: register the unified IM send APIs; when IM credentials exist, apply QQ / Feishu / Telegram / WeChat App ID, Secret, or Token fields, then configure the shared inbox path (`/fatfs/inbox/`) plus max attachment size. Platform groups (`cap_im_qq`, `cap_im_feishu`, `cap_im_tg`, `cap_im_wechat`) own connection lifecycle and register private send backends with the Gateway.
...
6. **Configure the unified IM Gateway**

    Register the `cap_im_gateway` APIs `send_message`, `send_image`, and `send_file`. Each IM platform privately registers a backend for channels such as `qq`, `feishu`, `telegram`, `wechat`, and `web`; Event Router only calls the unified API, and the Gateway selects a backend using `channel + chat_id`.
