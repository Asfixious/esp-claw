//! Integration tests for the default [`LlmCompactor`].
//!
//! Gated on the `llm` feature (on by default). A canned `ClawHttp` stands in for
//! the network so the test is hermetic.
#![cfg(feature = "llm")]
